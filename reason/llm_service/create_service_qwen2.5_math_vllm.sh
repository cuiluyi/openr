...
MODEL_BASE=/data/cuiluyi/resources/models/Qwen
CUDA_DEVICE_BASE=5
...
do
  ...
  tmux send-keys "CUDA_VISIBLE_DEVICES=5 $PYTHON_EXECUTABLE -m reason.llm_service.workers.vllm_worker --model-path $MODEL_PATH --controller-address http://$HOST_ADDR:$CONTROLER_PORT --host $HOST_ADDR --port $WORKER_PORT --worker-address http://$HOST_ADDR:$WORKER_PORT" Enter
done
