...
LANGUAGE_MODEL_NAME=openr/ckpts/DeepSeek-R1-Distill-Qwen-1.5B/sft2-optim_paths-wrong_data_amc_aime_batchsize32/checkpoint-640
...
do
  ...
  tmux send-keys "VLLM_USE_V1=0 CUDA_VISIBLE_DEVICES=$((i+CUDA_DEVICE_BASE)) $PYTHON_EXECUTABLE -m reason.serve.vllm_worker --model-path $LANGUAGE_MODEL_PATH --controller-address http://$HOST_ADDR:$CONTROLER_PORT --host $HOST_ADDR --port $WORKER_PORT --worker-address http://$HOST_ADDR:$WORKER_PORT" Enter
done
