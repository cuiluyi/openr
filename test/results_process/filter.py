...
def check_partial_correct(a, b, c, d, ground_truth, ans_list):
    if a + b + c + d != 0:
        return False
    for ans in ans_list:
        if task_module.judge_correct("", ans, ground_truth):
            return True
    return False
...
#     for obj in data:
    # 后续操作
        ground_answer = task_module.extract_answer(obj["groundtruth"])

        ans_list = [task_module.extract_answer(ans["text"]) for ans in obj["output"]]
