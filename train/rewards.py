...
def format_reward(completions, **kwargs):
    """
    Strictly checks if the output conforms to the alternating <think> and <step> format
    with matching pairs of tags.
    """
    completion_contents = [completion[0]["content"] for completion in completions]
    rewards = []

    for content in completion_contents:
        # Find all complete think-step pairs
        think_blocks = re.findall(r"<think>\n(.*?)\n</think>", content, re.DOTALL)
        step_blocks = re.findall(r"<step>\n(.*?)\n</step>", content, re.DOTALL)

        # Check if the counts match
        if len(think_blocks) != len(step_blocks):
            rewards.append(0.0)
            continue

        # Check if they strictly alternate
        is_valid = True
        think_positions = [m.start() for m in re.finditer(r"<think>\n", content)]
        step_positions = [m.start() for m in re.finditer(r"<step>\n", content)]

        if len(think_positions) != len(step_positions) or len(think_positions) == 0:
            rewards.append(0.0)
            continue

        # Verify that each think is followed by a step
        for i in range(len(think_positions)):
            if i < len(step_positions) - 1:  # Not the last pair
                if (
                    think_positions[i] > step_positions[i]
                    or step_positions[i] > think_positions[i + 1]
                ):
                    is_valid = False
                    break
            else:  # Last pair
                if think_positions[i] > step_positions[i]:
                    is_valid = False
                    break
...
def tag_count_reward(completions, **kwargs) -> list[float]:
    """
    Reward function: Counts the number of valid think-step pairs.
    Reward formula: min(1.0, valid_pairs / 3)
    Now only counts consecutive valid pairs.
    """

    def count_valid_pairs(text: str) -> float:
        # Use regex to match consecutive think-step pairs
        pattern = r"<think>\n.*?\n</think>\s*<step>\n.*?\n</step>"
        pairs = re.findall(pattern, text, re.DOTALL)
        valid_pairs = len(pairs)
        return min(1.0, valid_pairs / 3.0)

    contents = [completion[0]["content"] for completion in completions]
    return [count_valid_pairs(c) for c in contents]
    ...
    evals = [
        loop.create_task(
            run_catch_exceptions(
                score_subtask(
                    piston_client, problem_data, code, test_batch_size=test_batch_size
                )
            )
        )
...
def binary_code_reward(
    completions, num_parallel: int = 2, e2b_router_url=None, **kwargs
) -> list[float]:
    rewards = code_reward(
        completions, num_parallel=num_parallel, e2b_router_url=e2b_router_url, **kwargs
    )
...
def run_async_from_sync(
    scripts: list[str], language: str, num_parallel: int
) -> list[float]:
    ...
    async with semaphore:
        try:
            ...
            execution = await asyncio.wait_for(
                sandbox.run_code(script, language=language), timeout=ASYNCIO_TIMEOUT
            )
            ...
            print(
                f"Error in `run_script` from E2B sandbox ID {sandbox.sandbox_id} : {e}"
            )
            return 0.0
        finally:
            try:
...
def get_reward_funcs(script_args) -> list[Callable]:
    REWARD_FUNCS_REGISTRY = {
        ...
        "ioi_code": update_wrapper(
            partial(
                ioi_code_reward, test_batch_size=script_args.code_eval_test_batch_size
            ),
            ioi_code_reward,
        ),
