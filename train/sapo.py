...
from train.configs import GRPOConfig, GRPOScriptArguments
...
def main(script_args, training_args, model_args):
    ...
    def func(item):
        if "problem" not in item:
            item["problem"] = item["messages"][0]["content"]
        if "solution" not in item:
            item["solution"] = item["messages"][1]["content"]
        return item
    ...
    kwargs = {
        "dataset_name": "/".join(script_args.dataset_name.split("/")[-2:]),
        "tags": ["openr"],
    }
