...
class RepeatSampler(Sampler):
    ...
    mini_repeat_count = 3
         ...
         [0,  0,  0,  1,  1,  1,  2,  2,  2,  3,  3,  3,      |
          ...
          0,  0,  0,  1,  1,  1,  2,  2,  2,  3,  3,  3,      |
          4,  4,  4,  5,  5,  5,  6,  6,  6,  7,  7,  7,      |
          8,  8,  8,  9,  9,  9, 10, 10, 10, 11, 11, 11, trl..] |
          ---------   ---------   ---------   ---------
           ---------   ---------   ---------   ---------
            ---------   ---------   ---------   ---------
                         batch_size = 12
...
class GRPOTrainer(Trainer):
    ...
    def get_completion_ids(self, inputs: list[dict[str, Union[torch.Tensor, Any]]]) -> list[list[int]]:
        per_device_train_batch_size = self.args.per_device_train_batch_size
        steps_per_generation = self.args.steps_per_generation

        completion_ids = []
        for i in range(steps_per_generation):
            start = i * per_device_train_batch_size
            for gen_text in inputs[start]["generations"]:
                completion_ids.append(self.processing_class(gen_text, add_special_tokens=False)["input_ids"])
    ...
    def _generate_and_score_completions(
        ...
        if self.use_vllm:
            ...
            # Generate completions using colocated vLLM instances: each device holds vLLM copy and work on their own batch of prompts
                ...
                completion_ids = self.get_completion_ids(inputs)
